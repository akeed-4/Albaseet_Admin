...
export class AddCustomersComponent implements OnInit {
  ...
  dropDownFilter: FormControl = new FormControl();
  isLoading: boolean
  setDateRegex = /^[0-9]*$/;
...
//Phone number is required
  ...
  ngOnInit(): void {
   ...
   this.dirc  = this.currentLanguage === 'ar' ? 'rtl' : 'ltr';
    $(document).ready(function () {
      ...
      $('#customer_taxid').keypress(function () {
        var inputValue1 = (<HTMLInputElement>document.getElementById('customer_taxid')).value;
        if (inputValue1.length == 15) {
          return false
        };
        return true
      });
    ...
    this.customerForm = this.fb.group({
      ...
      customer_taxid: ['', [ Validators.required, Validators.pattern(("[0-9]\\d{14}"))  ]],
      customer_city: [''],
      customer_state: [''],
      customer_postcode: [''],
    });
...
filterCities(term:string){
  this.contryes = term
  ? this.contryes.filter(
      (x) =>
        x.arabic_name.toLowerCase().includes(term.toLowerCase()) ||
        x.english_name.toLowerCase().includes(term.toLowerCase())
    )
  : this.contryes;
}
  Addcustmer() {
    if (this.customerForm.valid) {
      ...
      if (this.id > 0) {
        ...
        this.services.Editcustomer(this.customers ).subscribe((doctor: any) => {
          ...
          this.errors = parseWebAPIErrors(erorr);
            this.isLoading = false;
            this.notifyService.showError(erorr);
        })
  ...
  filterMyOptions(term:string){
    alert(term)
    this.countriesFiltered = term ? this.contryes.filter(x => x.arabic_name?.toLowerCase()?.includes(term.toLowerCase())) : this.contryes;
  }
  GoToList() {
